...
test('Extract YAML frontmatter fields based on configuration.', async ({ expect }) => {
	const markdown_path = np.resolve(process.cwd(), 'src/__test__/samples/yaml.mdx');
	const markdown = await fs.readFile(markdown_path, { encoding: 'utf-8' });

	const mdast = prepare(markdown);
	const strings = extract_mdast_strings(mdast, config);
...
test('Ignore nodes base on configuration.', async ({ expect }) => {
	const markdown_path = np.resolve(process.cwd(), 'src/__test__/samples/ignore-nodes.mdx');
	const markdown = await fs.readFile(markdown_path, { encoding: 'utf-8' });
...
test('Extract JSX element attributes and children based on configuration.', async ({ expect }) => {
	const markdown_path = np.resolve(process.cwd(), 'src/__test__/samples/jsx.mdx');
	const markdown = await fs.readFile(markdown_path, { encoding: 'utf-8' });
...
test.only('Extact strings from a markdown list.', async ({ expect }) => {
	const markdown_path = np.resolve(process.cwd(), 'src/__test__/samples/list.mdx');
	const markdown = await fs.readFile(markdown_path, { encoding: 'utf-8' });
